...
/**
 * Progress event emitted by Docker during a build. Similar to pull events
 * but includes build-specific fields like stream, error, and aux.
 */
export interface BuildProgressEvent {
	stream?: string;
	error?: string;
	errorDetail?: { message: string };
	aux?: { ID: string }; // Image ID after successful build
	status?: string;
	progress?: string;
	id?: string;
}
...
export class ImageManager {
	...
	/**
	 * Build an image from a Dockerfile. Emits progress events to `onProgress`.
	 * Resolves with the inspected image once the build completes successfully.
	 *
	 * @param dockerfileContent - The content of the Dockerfile
	 * @param repository - Target image repository name
	 * @param tag - Target image tag
	 * @param buildArgs - Optional build arguments for ARG instruction
	 * @param onProgress - Callback for build progress events
	 */
	async build(
		...
		// Create build context as a stream
		const tarBuffer = await this.createBuildContext(dockerfileContent);
		...
		try {
			// Start the build - returns a stream
			const stream = this.docker.buildImage(tarBuffer, options) as unknown as NodeJS.ReadableStream;
			...
			await new Promise<void>((resolve, reject) => {
				...
				stream.on('data', (chunk: Buffer) => {
					...
					for (const line of lines) {
						try {
							const event: BuildProgressEvent = JSON.parse(line);
							if (event.error) {
								onProgress(event);
							} else if (event.stream) {
								const streamStr = event.stream.trim();
								if (streamStr && streamStr !== '\n') {
									onProgress({ stream: streamStr });
								}
							} else if (event.aux?.ID) {
								onProgress({ aux: event.aux });
							} else if (event.status) {
								onProgress({ status: event.status });
							}
						} catch (parseError) {
							// Not JSON, just emit as stream
							if (line.trim()) {
				...
				stream.on('error', (err: Error) => {
					reject(new Error(`Build stream error for ${repoTag}: ${err.message}`));
				});
			...
			const inspected = await this.inspect(repoTag);
			if (!inspected) {
				throw new Error(`Built image ${repoTag} not found after build`);
			}
			...
			if (err.statusCode === 404 || err.message?.includes('not found')) {
				throw new Error(`Base image not found during build: ${err.message}`);
			}
			throw new Error(`Build failed for ${repoTag}: ${err.message}`);
		}
	}

	/**
	 * Create a tarball build context containing just the Dockerfile.
	 * Docker requires a tarball even for single-file builds.
	 */
	private async createBuildContext(dockerfileContent: string): Promise<NodeJS.ReadableStream> {
		// Create a tar stream with just the Dockerfile
		// Using a simple approach with tar module
		const tar = require('tar');
		const { Readable } = require('stream');

		// Create a readable stream from the Dockerfile content
		const dockerfileStream = Readable.from([Buffer.from(dockerfileContent)]);
		...
		return pack as NodeJS.ReadableStream;
	}

	/**
	 * Validate a Dockerfile by checking for common syntax errors.
	 * This is a basic validation - full validation requires attempting a build.
	 */
	validateDockerfile(dockerfileContent: string): { valid: boolean; errors: string[]; warnings: string[] } {
		const errors: string[] = [];
		...
		const hasFrom = lines.some(line => {
			...
			return trimmed.toUpperCase().startsWith('FROM');
		});
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (upperLine.startsWith('EXPOSE')) {
				...
				if (!portMatch) {
					warnings.push(`Line ${i + 1}: EXPOSE instruction should specify a port number`);
				}
			}

			// Check for CMD or ENTRYPOINT
			if (upperLine.startsWith('CMD') || upperLine.startsWith('ENTRYPOINT')) {
				hasCmdOrEntrypoint = true;
			}

			// Check for deprecated MAINTAINER instruction
			if (upperLine.startsWith('MAINTAINER')) {
				warnings.push(`Line ${i + 1}: MAINTAINER is deprecated; use LABEL maintainer instead`);
			}

			// Check for ADD vs COPY
			if (upperLine.startsWith('ADD ')) {
				warnings.push(`Line ${i + 1}: Consider using COPY instead of ADD for better clarity`);
			}
		}

		// Warn about missing common instructions
		if (!hasExposedPort) {
			warnings.push('No EXPOSE instruction found - consider exposing the application port');
		}

		if (!hasCmdOrEntrypoint) {
			warnings.push('No CMD or ENTRYPOINT instruction found - container may not start properly');
		}
